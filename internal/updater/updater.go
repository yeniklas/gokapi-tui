...
const releaseAPI = "https://api.github.com/repos/yeniklas/gokapi-tui/releases/latest"
...
func Run(currentVersion string) error {
	...
	if rel.TagName == currentVersion {
		fmt.Printf("gokapi-tui is already up to date (%s)\n", currentVersion)
		return nil
	}

	assetName := fmt.Sprintf("gokapi-tui-%s-%s-%s", rel.TagName, runtime.GOOS, runtime.GOARCH)
	...
	probe, err := os.CreateTemp(dir, ".gokapi-tui-update-*")
	if err != nil {
	...
	fmt.Printf("Update gokapi-tui %s → %s? [y/N] ", currentVersion, rel.TagName)
	...
	tmp, err := os.CreateTemp(dir, ".gokapi-tui-update-*")
	if err != nil {
